在中心节点进行: 
    ...
    for 每一全局模型更新轮次 t = 1,2 ...:
        ...
        for 每一参与方 k in C_t 并行地:
            ...
            将更新后的模型参数 [[w^k_(t+1)]] 以及相关损失函数L^k_(i+1)发送给协调方
        协调方将收到的模型参数进行聚合，即对收到的加密模型参数使用加权平均
        if 协调方检查损失函数L已经收敛 or 已达到最大训练次数:
            协调方给各参与方发信号，使其全部停止模型训练。 
        协调方将聚合后的模型参数[[wb_(t+1)]]广播给所有参与方。 
        

在参与方进行:#参与方工作函数
    解密[[wb_t]]以获得wb_t
    从服务器获得最新的模型参数w^k_(1,1) = wb_t
    ...
    获得本地模型参数更新w^k_(t+1) = w^k_(B,S),进行加密,将[[w^k_(t+1)]]和损失函数L其发送给协调方
